...
public class DashboardService {
    ...
    public ClientDashboardResponse getClientDashboard(Integer clientId) {
        ...
        double totalOutstanding = pendingInvoices.stream()
                .mapToDouble(inv -> inv.getBalanceAmount() != null
                        ? inv.getBalanceAmount().doubleValue() : 0.0)
                .sum();
        ...
        Integer daysUntilDue = (nextDueDate != null && !nextDueDate.isBefore(now))
                ? (int) now.until(nextDueDate, java.time.temporal.ChronoUnit.DAYS) : null;
        ...
        if (contractEnd != null) {
            ...
            if (daysToExpiry < 0)        contractStatus = "EXPIRED";
            else if (daysToExpiry <= 30) contractStatus = "EXPIRING_SOON";
        }
        ...
        dash.setOicCount(client.getOicCount());
        dash.setJsoCount(client.getJsoCount());
        ...
        dash.setCurrentInvoiceAmount(currentInvoice != null && currentInvoice.getTotalAmount() != null
                ? currentInvoice.getTotalAmount().doubleValue() : null);
    ...
    private long nvl(Long val)     { return val != null ? val : 0L; }
}