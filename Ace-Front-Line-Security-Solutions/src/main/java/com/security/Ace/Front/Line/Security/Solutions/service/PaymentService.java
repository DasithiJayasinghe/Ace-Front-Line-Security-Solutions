...
import com.security.Ace.Front.Line.Security.Solutions.entity.PaymentMethod;
...
public class PaymentService {
    ...
    public PaymentResponse uploadPayment(PaymentUploadRequest request) {
        ...
        // Validate file type — guide: JPG/PNG/PDF only
        String contentType = request.getPaymentProof().getContentType();
        if (contentType == null || (!contentType.contains("image/jpeg")
                && !contentType.contains("image/png")
                && !contentType.contains("application/pdf"))) {
            throw new InvalidOperationException("Payment proof must be JPG, PNG, or PDF.");
        }
    ...
    public PaymentResponse verifyPayment(PaymentVerificationRequest request) {
        ...
        if (newStatus == VerificationStatus.VERIFIED) {
            ...
            emailService.sendPaymentVerifiedEmail(invoice.getClient(), payment);
    ...
    private PaymentResponse mapToResponse(Payment p) {
        ...
        if (p.getInvoice() != null) {
            r.setInvoiceTotal(p.getInvoice().getTotalAmount() != null
                    ? p.getInvoice().getTotalAmount().doubleValue() : null);
