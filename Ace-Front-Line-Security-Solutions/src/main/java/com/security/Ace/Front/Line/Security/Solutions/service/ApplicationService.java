...
public class ApplicationService {
    ...
    public JobApplicationDTO submitApplication(JobApplicationDTO applicationDTO, MultipartFile cvFile, MultipartFile certificateFile) {
        ...
        JobVacancy vacancy = vacancyRepository.findById(applicationDTO.getVacancyId())
                .orElseThrow(() -> new ResourceNotFoundException("Vacancy not found with id: " + applicationDTO.getVacancyId()));

        // Check if application already exists for this email and vacancy
        if (applicationRepository.findByEmailAndVacancyId(applicationDTO.getEmail(), applicationDTO.getVacancyId()).isPresent()) {
    ...
    public List<JobApplicationDTO> getApplicationsByVacancyAndStatus(Long vacancyId, String status) {
        ...
        List<JobApplication> applications = applicationRepository.findByVacancyIdAndApplicationStatus(vacancyId, appStatus);
        return applications.stream().map(this::convertToDTO).collect(Collectors.toList());
    }
    ...
    /**
     * Get CV list for operational manager (all applications with job title included)
     */
