...
public class ShiftScheduleController {
    ...
    public ResponseEntity<Void> assignOfficersToShift(
        ...
        scheduleService.assignOfficersToShift(id, request);
        return ResponseEntity.ok().build();
    }

    @DeleteMapping("/assignments/{assignmentId}")
    public ResponseEntity<Void> removeAssignment(@PathVariable Long assignmentId) {
        scheduleService.removeAssignment(assignmentId);
        return ResponseEntity.ok().build();
    }
    ...
    public ResponseEntity<ShiftScheduleDTO> getScheduleByCompanyAndMonthAndYear(
        ...
        return ResponseEntity.ok(
                scheduleService.getScheduleByCompanyAndMonthAndYear(clientCompanyId, month, year)
        );
    }
    ...
    public ResponseEntity<List<ShiftScheduleDTO>> getAreaManagerApprovedHistory() {
        User currentUser = authService.getCurrentUser();
        return ResponseEntity.ok(
                scheduleService.getAreaManagerApprovedHistory(currentUser.getId())
        );
    }
    ...
    @GetMapping("/client/{companyId}/current")
    public ResponseEntity<List<ShiftScheduleDTO>> getClientCurrentSchedules(@PathVariable Long companyId) {
        return ResponseEntity.ok(scheduleService.getClientCurrentSchedules(companyId));
    }
    ...
    public ResponseEntity<List<ShiftScheduleDTO>> getFilteredSchedules(
        ...
        return ResponseEntity.ok(scheduleService.getFilteredSchedules(branch, company, month));
    }
}