...
const operationalManagerItems = [
  ...
  { label: "Recruitment", path: "careers" },
...
const executiveOfficerItems = [
  ...
  { label: "Request a Leave", path: "request-leave" },
  { label: "Inquiry Documents", path: "inquiry-documents" },
  { label: "Announcements", path: "announcements" },
];
...
function renderDashboardRoutes(items: { label: string; path: string }[]) {
  // some dashboard pages (careers/interviews) have custom components defined
  // so we avoid generating placeholder routes for them – otherwise the
  // placeholder would shadow the real page and the manager would never see
  // the actual UI.
  const skip = new Set(["careers", "interviews", "inquiry-management", "announcements", "inquiry-documents"]);

  return (
    <>
      <Route index element={<PlaceholderPage title="Dashboard" />} />
      {items
        .filter((item) => !skip.has(item.path))
        .map((item) => (
    ...
    <TooltipProvider>
      ...
      <BrowserRouter>
        <Routes>
          ...
          {/* accept common typo and redirect */}
          ...
          <Route path="/operational-manager" element={<DashboardLayout title="Operational Manager" role="Operational Manager" items={operationalManagerItems} basePath="/operational-manager" />}>
            ...
            <Route path="careers" element={<OperationalCareers />} />
            <Route path="interviews" element={<Interviews />} />
            <Route path="inquiry-management" element={<OperationalInquiries />} />
          </Route>
          ...
          <Route path="/executive-officer" element={<DashboardLayout title="Executive Officer" role="Executive Officer" items={executiveOfficerItems} basePath="/executive-officer" />}>
            ...
            <Route path="interviews" element={<Interviews />} />
            <Route path="inquiry-documents" element={<AdminInquiries />} />
            <Route path="announcements" element={<Announcements />} />
          </Route>
