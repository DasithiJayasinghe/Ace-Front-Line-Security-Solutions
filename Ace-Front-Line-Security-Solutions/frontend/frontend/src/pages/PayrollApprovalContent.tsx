...
export default function PayrollApprovalContent() {
  ...
  const handleApprovePayroll = async () => {
    ...
    try {
      ...
      setPendingPayrolls((prev) => prev.filter((p) => p.id !== selectedPayroll.id));
  ...
  const handleRejectPayroll = async () => {
    ...
    setIsProcessing(true);
    try {
      await admin_payrollService.rejectPayroll(selectedPayroll.id, rejectionReason);
  ...
  const calculateGrossSalary = (payroll: PayrollResponse) => {
    return payroll.basicSalary + editingAllowances + payroll.otAmount;
  };

  const calculateNetSalary = (payroll: PayrollResponse) => {
    const gross = calculateGrossSalary(payroll);
    const totalDeductions =
      payroll.loanDeduction + payroll.advanceDeduction + payroll.otherDeductions;
                  ...
                  {pendingPayrolls.map((payroll) => (
                    <TableRow key={payroll.id}>
                      ...
                      <TableCell className="text-right">
                        LKR {(payroll.basicSalary + payroll.allowances + payroll.otAmount).toLocaleString()}
                      </TableCell>
              ...
              <div className="space-y-3 pb-4 border-b">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-medium">LKR {selectedPayroll.otAmount.toLocaleString()}</p>
                  </div>
                </div>
              ...
              <div className="space-y-3 pb-4 border-b">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    ...
                    <p className="font-medium">LKR {selectedPayroll.loanDeduction.toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Advance</p>
                    <p className="font-medium">LKR {selectedPayroll.advanceDeduction.toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Other</p>
                    <p className="font-medium">LKR {selectedPayroll.otherDeductions.toLocaleString()}</p>
                  </div>
                </div>
              </div>
