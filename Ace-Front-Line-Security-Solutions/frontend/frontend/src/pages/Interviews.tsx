...
const Interviews = () => {
  ...
  // Stats
  const totalCandidates = useMemo(() => interviews.reduce((sum, i) => sum + i.applicants.length, 0), [interviews]);
  const selectedCount = useMemo(() => interviews.reduce((sum, i) => sum + i.applicants.filter(a => a.applicationStatus === "SELECTED").length, 0), [interviews]);
  ...
  const groupedSessions = useMemo(() => {
    ...
    return Array.from(map.entries()).map(([key, records]) => {
      ...
      const totalApplicants = records.reduce((s, r) => s + r.applicants.length, 0);
      const selectedApplicants = records.reduce((s, r) => s + r.applicants.filter(a => a.applicationStatus === "SELECTED").length, 0);
    ...
    <div className="space-y-6">
      ...
      <div className="relative rounded-2xl overflow-hidden bg-[#1A1A1B] p-5 text-white shadow-xl">
        ...
        <div className="relative flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-[#FFD700]/15 rounded-xl border border-[#FFD700]/20">
              <CalendarClock className="h-5 w-5 text-[#FFD700]" />
            </div>
            <div>
              <h2 className="text-xl font-bold tracking-tight">Interview Management</h2>
              <p className="text-sm text-white/50">Track and manage all scheduled interviews</p>
            </div>
          </div>
        </div>
        ...
        <div className="relative grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="bg-white/[0.06] backdrop-blur-sm rounded-xl p-3 text-center border border-white/[0.06] hover:bg-white/[0.1] transition-colors">
            ...
            <p className="text-[10px] text-white/40 uppercase tracking-widest mt-0.5">Interviews</p>
          </div>
          <div className="bg-white/[0.06] backdrop-blur-sm rounded-xl p-3 text-center border border-white/[0.06] hover:bg-white/[0.1] transition-colors">
            <Users className="h-4 w-4 mx-auto mb-1.5 text-purple-400" />
            <p className="text-xl font-bold">{totalCandidates}</p>
            <p className="text-[10px] text-white/40 uppercase tracking-widest mt-0.5">Total Candidates</p>
          </div>
          <div className="bg-white/[0.06] backdrop-blur-sm rounded-xl p-3 text-center border border-white/[0.06] hover:bg-white/[0.1] transition-colors">
            <Clock className="h-4 w-4 mx-auto mb-1.5 text-amber-400" />
            <p className="text-xl font-bold">{pendingCount}</p>
            <p className="text-[10px] text-white/40 uppercase tracking-widest mt-0.5">Awaiting Selection</p>
          </div>
          <div className="bg-white/[0.06] backdrop-blur-sm rounded-xl p-3 text-center border border-white/[0.06] hover:bg-white/[0.1] transition-colors">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex items-center gap-2">
          <Filter className="h-4 w-4 text-[#FFD700] flex-shrink-0" />
          <select
        ...
        <div className="flex items-center gap-2">
          <ArrowUpDown className="h-4 w-4 text-[#FFD700] flex-shrink-0" />
          ...
          {([
            ...
            <Button
              ...
              className={sortBy === s.value ? "bg-[#1A1A1B] hover:bg-[#FFD700] hover:text-[#1A1A1B] text-white text-xs h-7 px-3" : "text-xs h-7 px-3 border-[#FFD700]/30 hover:bg-[#FFD700]/10"}
                  ...
                  {session.allRoles.length > 0 && (
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {session.allRoles.map(role => (
                        <span key={role} className="text-xs bg-[#1A1A1B]/10 text-[#1A1A1B] px-2 py-0.5 rounded-full font-medium">
                ...
                {session.records.map(interview => (
                  <div key={interview.id} className="space-y-3">
                    ...
                    <div className="flex items-center gap-2.5 pb-2 border-b border-gray-100">
                      <div className="h-7 w-7 rounded-lg bg-[#FFD700]/10 flex items-center justify-center">
                        ...
                        <div key={app.applicationId} className={`flex items-start gap-4 p-4 rounded-xl border transition-colors ${
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            ...
                            {app.applicationStatus !== "SELECTED" && (
                              <Button size="sm" onClick={() => openSelectModal(app)} className="bg-green-600 hover:bg-green-700 text-white">
          ...
          {selectTarget && (
            <div className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <Input id="report-date" type="date" value={reportDate} onChange={e => setReportDate(e.target.value)} required className="h-11" />
              </div>
              ...
              <div className="flex gap-3">
                ...
                <Button className="flex-1 bg-green-600 hover:bg-green-700 text-white" onClick={handleConfirmSelect} disabled={selectLoading}>
