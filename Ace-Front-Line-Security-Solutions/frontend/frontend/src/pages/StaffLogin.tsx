...
// available staff roles; the operational manager button was accidentally removed previously
const roles = [
  ...
  { id: "operational-manager", label: "Operational Manager", icon: Building2, desc: "Handle vacancies, inquiries & operations" },
...
const StaffLogin = () => {
  ...
  // note: although we label it email in the UI, the backend expects a username
  // we provide a few convenient aliases below so people can enter familiar
  // addresses like ops@ace.com and still authenticate.
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // map known email aliases to actual usernames used by the backend
      let username = email.trim();
      const aliasMap: Record<string,string> = {
        "ops@ace.com": "ops_manager",
        "admin@ace.com": "admin",
        "director@ace.com": "director",
        "exec@ace.com": "exec",
      };
      if (aliasMap[username.toLowerCase()]) {
        username = aliasMap[username.toLowerCase()];
      }

      // Use the dedicated auth endpoint for login
