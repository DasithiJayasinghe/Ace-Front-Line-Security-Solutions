    ...
    >
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-sm font-semibold text-foreground mb-1 leading-tight">{title}</h3>
          <p className="text-xs text-muted-foreground leading-relaxed">{description}</p>
          {buttonText && (
            <button
              onClick={disabled ? undefined : onClick}
              disabled={disabled}
              className={`mt-3 inline-flex items-center gap-1.5 px-3 py-2 rounded-md text-xs font-semibold transition-all ${
                disabled
