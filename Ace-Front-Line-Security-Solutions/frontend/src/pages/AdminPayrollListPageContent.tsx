...
export default function AdminPayrollListPageContent() {
  ...
  const handleProceedAllToBank = async () => {
    ...
    try {
      ...
      const data = payrolls.map((p) => {
        const grossSalary = p.basicSalary + p.allowances + (p.otAmount || 0);
        return {
          ...
          "Loan Deduction": p.loanDeduction || 0,
  ...
  const totalGross = filteredPayrolls.reduce((sum, p) => sum + (p.basicSalary + p.allowances + (p.otAmount || 0)), 0);
  const totalDeductions = filteredPayrolls.reduce((sum, p) => sum + (p.loanDeduction || 0) + (p.advanceDeduction || 0), 0);
                    ...
                    <TableBody>
                      {filteredPayrolls.map((payroll) => (
                        <TableRow key={payroll.id} className="border-border/60 hover:bg-muted/50">
                          ...
                          <TableCell className="font-semibold">LKR {(payroll.basicSalary + payroll.allowances + (payroll.otAmount || 0)).toLocaleString()}</TableCell>
                          <TableCell className="font-semibold">LKR {((payroll.loanDeduction || 0) + (payroll.advanceDeduction || 0)).toLocaleString()}</TableCell>
              ...
              <div className="bg-muted/50 rounded-lg p-4">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">LKR {(selectedPayroll.basicSalary + selectedPayroll.allowances + (selectedPayroll.otAmount || 0)).toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Loan Deduction</span>
                    <span className="font-medium">LKR {(selectedPayroll.loanDeduction || 0).toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Advance Deduction</span>
                    <span className="font-medium">LKR {(selectedPayroll.advanceDeduction || 0).toLocaleString()}</span>
                  </div>
