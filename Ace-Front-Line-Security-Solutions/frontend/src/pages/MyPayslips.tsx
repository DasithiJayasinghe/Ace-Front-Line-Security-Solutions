...
const MyPayslips = ({ mode = "history" }: MyPayslipsProps) => {
    ...
    const { data: payslips = [], isLoading, isError } = useQuery({
        ...
        queryFn: async () => {
            if (!userId) return [];
            const response = await fetch(`http://localhost:8080/api/payroll/officer/payslips/${userId}`);
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-3xl shadow-sm border border-neutral-100 relative overflow-hidden">
                ...
                <div className="flex items-center gap-3">
                    <div className="text-right hidden sm:block">
                        <p className="text-sm font-bold text-neutral-700 uppercase tracking-tighter">{localStorage.getItem("fullName") || "User Name"}</p>
                        <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest text-primary">{localStorage.getItem("userRole") || "Officer"}</p>
                    </div>
                    <div className="h-12 w-12 rounded-2xl bg-primary flex items-center justify-center text-black font-black shadow-lg shadow-primary/20 scale-105">
                        {fullName.charAt(0)}
                    </div>
                </div>
            </div>
