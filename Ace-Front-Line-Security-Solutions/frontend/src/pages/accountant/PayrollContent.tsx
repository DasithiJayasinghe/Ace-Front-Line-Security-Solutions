...
import { Loader, DollarSign, CheckCircle2, Clock, TrendingUp, Wallet, CreditCard, Calendar, FileText, ExternalLink } from "lucide-react";
...
export type PayrollSubTab = "salary-trend" | "generate-payroll" | "payroll-records" | "advance" | "loan";
...
export default function PayrollContent({ category, subTab }: PayrollContentProps) {
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Monthly payroll statistics for {currentMonth}</p>
          </div>
        </div>
        ...
        {statsLoading ? (
          <Card className="bg-card border border-border">
            <CardContent className="p-12 text-center">
              ...
              <p className="text-muted-foreground">Loading salary statistics...</p>
            </CardContent>
          </Card>
        ) : stats.length === 0 ? (
          <Card className="bg-card border border-border">
            <CardContent className="p-12 text-center">
              <DollarSign className="h-16 w-16 mx-auto mb-4 text-primary/50" />
              <p className="text-muted-foreground">No payroll data available for {currentMonth}</p>
            </CardContent>
          </Card>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {stats.map((stat, idx) => (
              <Card key={idx} className="bg-gradient-to-br from-primary/20 to-primary/10 border border-primary/20">
                <CardContent className="p-6">
                  <p className="text-sm text-muted-foreground">{stat.role} - {stat.payMonth}</p>
                  <p className="text-2xl font-bold text-primary mt-2">LKR {(stat.totalAmount || 0).toLocaleString()}</p>
                  <p className="text-xs text-muted-foreground mt-1">Avg: LKR {(stat.averageSalary || 0).toLocaleString()}</p>
                  <p className="text-xs text-muted-foreground">Count: {stat.totalProcessed}</p>
                </CardContent>
              </Card>
            ))}
          </div>
  ...
  // Handle salary-trend tab (security - Coming Soon)
  if (category === "security" && subTab === "salary-trend") {
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          <div className="p-2 rounded-xl bg-gradient-to-br from-yellow-400 to-yellow-600 shadow-lg">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Security Force payroll statistics</p>
          </div>
        </div>
        <Card className="bg-card border border-border">
          <CardContent className="p-12 text-center">
            ...
            <p className="text-muted-foreground">Security Force salary trend analysis is under development.</p>
          </CardContent>
        </Card>
      </div>
    );
  ...
  if (subTab === "generate-payroll") {
    const staffType = category === "admin" ? "Admin Staff" : "Security Force";

    // For admin, show embedded Admin Payroll Page Content
    if (category === "admin") {
      return <AdminPayrollPageContent />;
    }

    // Original form for Security Force (Coming Soon)
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          <div className="p-2 rounded-xl bg-gradient-to-br from-yellow-400 to-yellow-600 shadow-lg">
          ...
          <div>
            <h2 className="text-2xl font-bold bg-gradient-to-r from-yellow-300 to-yellow-500 bg-clip-text text-transparent">
              Generate {staffType} Payroll
            </h2>
            <p className="text-sm text-muted-foreground">Create payroll for {staffType.toLowerCase()}</p>
          </div>
        </div>
        ...
        <Card className="bg-card border border-border">
          <CardContent className="p-12 text-center">
            <Clock className="h-16 w-16 mx-auto mb-4 text-primary/50" />
            <h3 className="text-xl font-semibold text-foreground mb-2">Coming Soon</h3>
            <p className="text-muted-foreground">Security Force payroll generation is under development.</p>
          </CardContent>
        </Card>
      </div>
    );
  ...
  // Handle generate-payroll tab (security - Coming Soon)
  if (category === "security" && subTab === "generate-payroll") {
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          <div className="p-2 rounded-xl bg-gradient-to-br from-yellow-400 to-yellow-600 shadow-lg">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Security Force payroll generation</p>
          </div>
        </div>
        <Card className="bg-card border border-border">
          <CardContent className="p-12 text-center">
  ...
  if (subTab === "payroll-records") {
    // For admin, show embedded Admin Payroll List Page Content
    if (category === "admin") {
      return <AdminPayrollListPageContent />;
    }

    // Coming Soon for Security Force
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          <div className="p-2 rounded-xl bg-gradient-to-br from-yellow-400 to-yellow-600 shadow-lg">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Security Force payroll records</p>
          </div>
        </div>
        <Card className="bg-card border border-border">
          <CardContent className="p-12 text-center">
            <Clock className="h-16 w-16 mx-auto mb-4 text-primary/50" />
            <h3 className="text-xl font-semibold text-foreground mb-2">Coming Soon</h3>
            <p className="text-muted-foreground">Security Force payroll records are under development.</p>
          </CardContent>
        </Card>
      </div>
    );
