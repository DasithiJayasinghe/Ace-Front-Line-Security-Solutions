...
const AccountantInvoiceReview = () => {
    ...
    const serviceItems  = invoice.items.filter((i) => ["OIC_SERVICE","JSO_SERVICE","OVERTIME"].includes(i.itemType));
    const otherItems    = invoice.items.filter((i) => i.itemType === "OTHER_CHARGE");
    const deductItems   = invoice.items.filter((i) => i.itemType === "DEDUCTION");
    const taxItems      = invoice.items.filter((i) => ["SSCL","VAT","LATE_FEE"].includes(i.itemType));
    const displayItems  = invoice.items.filter((i) => !["SSCL","VAT","LATE_FEE"].includes(i.itemType));
    ...
    return (
        <div className="space-y-6 pb-8">
            ...
            <nav className="flex items-center gap-1.5 text-sm text-muted-foreground flex-wrap">
                <Link to="/accountant/dashboard" className="hover:text-primary">Accountant</Link>
                <ChevronRight className="w-3 h-3 shrink-0" />
                <Link to="/accountant/invoices" className="hover:text-primary">Invoices</Link>
                <ChevronRight className="w-3 h-3 shrink-0" />
                    ...
                    <div className="bg-card rounded-2xl border shadow-sm overflow-hidden print:shadow-none">
                        ...
                        <div className="p-6 border-b">
                            ...
                            <div className="mt-5 pt-4 border-t">
                                ...
                                {invoice.clientVatNo && (
                                    <p className="text-sm text-muted-foreground">VAT: {invoice.clientVatNo}</p>
                                )}
                            </div>
                        </div>
                                    ...
                                    {displayItems.map((item, idx) => (
                                        <tr key={item.itemId ?? idx}
                                            className={`border-b last:border-0 ${
                                                item.itemType === "DEDUCTION" ? "bg-red-50/50 text-red-700" :
                                                idx % 2 === 1 ? "bg-muted/10" : ""
                                            }`}>
