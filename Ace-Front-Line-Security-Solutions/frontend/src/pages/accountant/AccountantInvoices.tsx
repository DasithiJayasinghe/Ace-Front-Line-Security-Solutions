...
const statusConfig: Record<string, { label: string; cls: string }> = {
    ...
    ISSUED:              { label: "Issued",            cls: "bg-blue-100 text-blue-700 border border-blue-200" },
    PENDING:             { label: "Pending",           cls: "bg-yellow-100 text-yellow-700 border border-yellow-200" },
    PAYMENT_UPLOADED:    { label: "Verifying",         cls: "bg-purple-100 text-purple-700 border border-purple-200" },
    PAYMENT_REJECTED:    { label: "Proof Rejected",    cls: "bg-orange-100 text-orange-700 border border-orange-200" },
    VERIFICATION_PENDING:{ label: "Verifying",         cls: "bg-purple-100 text-purple-700 border border-purple-200" },
    PAID:                { label: "Paid",              cls: "bg-green-100 text-green-700 border border-green-200" },
    OVERDUE:             { label: "Overdue",           cls: "bg-red-100 text-red-700 border border-red-200" },
    DISPUTED:            { label: "Disputed",          cls: "bg-orange-100 text-orange-700 border border-orange-200" },
    WAIVED:              { label: "Waived",            cls: "bg-teal-100 text-teal-700 border border-teal-200" },
    CANCELLED:           { label: "Cancelled",         cls: "bg-gray-100 text-gray-400 border border-gray-200" },
};
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                {[
                    { label: "Total Invoices",   value: invoices.length,                  color: "text-foreground", bg: "" },
                    { label: "Total Revenue",    value: formatLKR(totalRevenue),           color: "text-foreground", bg: "" },
                    { label: "Outstanding",      value: formatLKR(totalOutstanding),       color: "text-red-600",    bg: "bg-red-50" },
                    { label: "Overdue",          value: overdueCount,                      color: overdueCount > 0 ? "text-red-600" : "text-foreground", bg: overdueCount > 0 ? "bg-red-50" : "" },
                ].map((s) => (
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap gap-3">
                        ...
                        <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)}
                            ...
                            <option value="PAYMENT_UPLOADED">Verifying</option>
                        ...
                        <button
                            ...
                            className="flex items-center gap-1.5 border rounded-xl px-3 py-2 text-sm hover:bg-muted/30 transition-colors"
                            title="Refresh"
                        >
            ...
            {activeTab === "queue" && (
                <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3">
                        <div>
                            <h3 className="font-bold text-lg">Draft Invoice Queue</h3>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        <div className="rounded-xl border p-3 text-center">
                            ...
                            <p className="text-2xl font-black mt-1">{queueInvoices.length}</p>
                        </div>
                        <div className="rounded-xl border p-3 text-center">
                            <p className="text-xs text-muted-foreground font-semibold uppercase">Total Amount</p>
                            <p className="text-xl font-black mt-1 text-yellow-600">{formatLKR(queueInvoices.reduce((s, i) => s + (i.totalAmount ?? 0), 0))}</p>
                        </div>
                        <div className="rounded-xl border p-3 text-center">
                            <p className="text-xs text-muted-foreground font-semibold uppercase">Auto-Generated</p>
                            <p className="text-2xl font-black mt-1">{queueInvoices.filter((i) => i.invoiceType === "AUTO").length}</p>
                        </div>
                    </div>
                    ...
                    {queueLoading ? (
                        ...
                        <div className="text-center py-20 text-muted-foreground border-2 border-dashed rounded-2xl">
                            ...
                            <p className="text-sm mt-1">
                                Click <strong>Generate Invoices</strong> to auto-generate for all active clients,
                                or <Link to="/accountant/invoices/create" className="text-yellow-600 font-semibold underline">create one manually</Link>.
                            </p>
                        </div>
                    ) : (
                        <div className="rounded-2xl border shadow-sm overflow-hidden">
