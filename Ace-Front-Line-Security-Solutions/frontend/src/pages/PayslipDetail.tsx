...
const PayslipDetail = () => {
    ...
    return <PayslipDetailContent id={Number(id)} onBack={() => navigate(-1)} />;
};
...
export const PayslipDetailContent = ({ id, onBack, showEdit = true }: { id: number, onBack?: () => void, showEdit?: boolean }) => {
    ...
    useEffect(() => {
        const fetchPayslip = async () => {
            try {
                const response = await fetch(`http://localhost:8080/api/payroll/${id}`);
                if (response.ok) {
    ...
    const handleDownload = async () => {
        try {
            ...
            const response = await fetch(`http://localhost:8080/api/payroll/payslip/pdf/${id}`);

            if (!response.ok) {
                const errorData = await response.text();
                console.error("PDF generation failed:", errorData);
                toast.error("Failed to generate PDF. Server returned an error.");
                return;
            }

            const blob = await response.blob();
            if (blob.size < 100) { // Unusually small for a PDF
            ...
            toast.error("Network error during PDF download");
        }
    };
        ...
        <div className="min-h-screen bg-neutral-50 p-4 md:p-8 animate-in fade-in duration-700 font-sans print:p-0 print:bg-white">
            ...
            <div className="max-w-[210mm] mx-auto space-y-6">
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 no-print bg-white p-4 rounded-xl border border-neutral-200 shadow-sm">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {showEdit && salary.status === 'CALCULATED' && (
                            <Button
                                onClick={() => navigate(`/accountant/payroll-records/edit/${salary.id}`)}
                        ...
                        <Button onClick={handlePrint} className="bg-neutral-800 hover:bg-neutral-900 text-white font-bold">
                        ...
                        <div className="p-8 border-b border-neutral-200">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-y-6 gap-x-8">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-bold text-neutral-900 mt-1">Field Assigned</p>
                                </div>
                                <div>
                                    ...
                                    <div className="border border-neutral-200 border-t-0 rounded-b-lg overflow-hidden">
                                        <table className="w-full text-xs">
                                            ...
                                            <tbody className="divide-y divide-neutral-100">
                                                ...
                                                <tr>
                                                    <td className="px-4 py-3 text-neutral-700">Overtime ({salary.totalShifts} shifts)</td>
                ...
                <div className="text-center no-print pb-8">
                    <p className="text-xs text-neutral-400 mb-4">Need a physical copy? Use the Print button above.</p>
                </div>
            </div>
        </div>
    );
