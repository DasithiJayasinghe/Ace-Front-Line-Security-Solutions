...
const PayrollRecords = () => {
    ...
    const fetchPayrollData = async (month: string) => {
        ...
        try {
            const [calcRes, paidRes] = await Promise.all([
                fetch(`http://localhost:8080/api/payroll/list?month=${queryMonth}`),
                fetch(`http://localhost:8080/api/payroll/history?month=${queryMonth}`)
            ]);
            ...
            toast.error("Failed to fetch payroll records from database.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleMarkPaid = async (id: string) => {
        try {
            const response = await fetch(`http://localhost:8080/api/payroll/pay/${id}`, {
                method: 'POST'
            });
            if (response.ok) {
                ...
                toast.error(`Failed to mark as paid: ${errorData}`);
            }
        } catch (error) {
            console.error("Error marking payroll as paid:", error);
            toast.error("Network error. Could not mark payroll as paid.");
        }
    };
    ...
    const handleDownloadPdf = (id: string) => {
        window.open(`http://localhost:8080/api/payroll/payslip/pdf/${id}`, '_blank');
        toast.info("Opening PDF payslip...");
    };
        ...
        <div className="max-w-7xl mx-auto space-y-8 pb-12 animate-in fade-in duration-1000">
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <Button
                        onClick={() => navigate("/accountant/generate-payroll")}
                                        ...
                                        <TableCell className="text-right pr-8 py-4">
                                            <div className="flex justify-end gap-2 transition-opacity">
                                                <Button
                                                    ...
                                                    title="View Payslip"
                                                    onClick={() => navigate(`/accountant/payslip/${s.id}`)}
                                                >
                                                ...
                                                <Button
                                                    ...
                                                    title="Edit"
                                                    onClick={() => navigate(`/accountant/payroll-records/edit/${s.id}`)}
                                                >
                                        ...
                                        <TableCell className="text-right pr-8 py-4">
                                            <div className="flex justify-end gap-2 transition-opacity">
                                                <Button
                                                    ...
                                                    className="h-8 w-8 text-neutral-400 hover:text-neutral-900"
                                                    title="View Payslip"
                                                    onClick={() => navigate(`/accountant/payslip/${p.id}`)}
                                                >
                                                    <Eye className="h-4 w-4" />
                                                </Button>
                                                <Button
