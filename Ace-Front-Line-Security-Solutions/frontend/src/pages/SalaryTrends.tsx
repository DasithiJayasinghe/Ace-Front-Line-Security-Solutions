...
const SalaryTrends = () => {
    ...
    useEffect(() => {
        fetch(`${API_BASE_URL}/officers/list`)
            .then(res => res.json())
    ...
    const fetchTrends = async () => {
        ...
        try {
            const url = officerFilter !== "all"
                ? `${API_BASE_URL}/payroll/trends?officerId=${officerFilter}`
                : `${API_BASE_URL}/payroll/trends`;
    ...
    const exportReport = () => {
        let url = `${API_BASE_URL}/payroll/trends/pdf`;
        if (officerFilter !== "all") {
            url += `?officerId=${officerFilter}`;
        }
