  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h3 className="text-xl font-bold text-foreground">Paysheet History</h3>
          <p className="text-sm text-muted-foreground">View and download your paysheets</p>
        </div>
      </div>
      ...
      {paysheets.length === 0 ? (
        <Card className="bg-card border-border/50">
          <CardContent className="p-12 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-2">Your paysheets will appear here once they are generated and processed.</p>
          </CardContent>
        </Card>
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <Card className="bg-gradient-to-br from-purple-500/15 to-purple-500/5 border-purple-500/20">
              <CardContent className="p-4">
                <p className="text-xs text-muted-foreground uppercase font-semibold">Total Paysheets</p>
                <p className="text-2xl font-bold text-purple-600 mt-1">{paysheets.length}</p>
              </CardContent>
            </Card>
            <Card className="bg-gradient-to-br from-green-500/15 to-green-500/5 border-green-500/20">
              <CardContent className="p-4">
                ...
                <p className="text-2xl font-bold text-green-600 mt-1">
                  {formatCurrency(paysheets.reduce((sum, p) => sum + (p.basicSalary + p.allowances), 0))}
                </p>
              </CardContent>
            </Card>
            <Card className="bg-gradient-to-br from-blue-500/15 to-blue-500/5 border-blue-500/20">
              <CardContent className="p-4">
                <p className="text-xs text-muted-foreground uppercase font-semibold">Avg. Net Salary</p>
                <p className="text-2xl font-bold text-blue-600 mt-1">
                  {formatCurrency(paysheets.reduce((sum, p) => sum + p.netSalary, 0) / paysheets.length)}
                </p>
              </CardContent>
            </Card>
          </div>
                  ...
                  <TableBody>
                    {paysheets.map((paysheet) => (
                      <TableRow key={paysheet.id} className="border-border/50 hover:bg-muted/40 transition-colors">
                        ...
                        <TableCell className="text-sm">{formatCurrency(paysheet.basicSalary)}</TableCell>
                        <TableCell className="text-sm">{formatCurrency(paysheet.allowances)}</TableCell>
                        <TableCell className="text-sm text-red-600">{formatCurrency(paysheet.deductions)}</TableCell>
                        ...
                        <TableCell>
                          <div className="flex gap-2">
                            <Button
                              ...
                              className="text-xs h-8 border-green-500/30 text-green-600 hover:bg-green-500/10"
                            >
